  ...
  #          self.end_mileage = self.starting_mileage + self.business_mileage
    ...
    def create_overtime_entries(self):
        ...
        if self.overtime_hours and not self.overtime_analytic_line_id:
            overtime_account = self.env.ref('magnus_timesheet.analytic_overtime_hours').id
            uom = self.env.ref('product.product_uom_hour').id
            analytic_line = analytic_line.create({
                'name':'/',
                'account_id':overtime_account,
                'date':self.date_to,
    ...
    def action_timesheet_confirm(self):
        ...
        for i in range(7):
            ...
            # stand by entries must be removed from logged 8 hrs
            hour = sum(self.env['account.analytic.line'].search(
                [('date', '=', date), ('sheet_id', '=', self.id), ('task_id.standby', '=', False)]).mapped(
                'unit_amount'))
